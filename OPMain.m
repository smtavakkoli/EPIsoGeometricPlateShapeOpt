...
% (X(i+1)=X(i)+d)
% X is vector of design varible (m x 1)
% m is total number of design varibles
% d = alpha*(dF-dG*lambda)
% alpha is step 
% dF is gradient vector of objective function (m x 1)
% dG is gradient matrix of constraint functions (m x n)
% n is total number of constraint functions
% lambda is vector of lagrang multiplier (n x 1)
% ' is transpose sign
    alpha=0.005;
    ii=2;
    G=0;
    ...
    %% Calculation of lagrangMultiplier
        ...
        if isnan(lambda)
            ...
            while alpha<1
                ...
                [K]=Assembelingstifness(Ndof,Numbering,KK,DOF);
                %% Plastic analysis
                [force,dis,sig,epsilon]=PlasticAnalysis(nli,...
                    PointDis,PointLoad,K,KK,detJ1,detJ2,...
                    guss_x,w_guss_x,guss_y,w_guss_y,DOF,...
                    Numbering,t,Dmat,Bmat,H,Ndof,noe,effective_stress);
                %% Claculate energy dissipation
    ...
    PlotShapeResults(DVI,FPI,noe,Numbering,norx,Ux,Uy,...
