function []=PlotShapeResults(DVI,FPI,noe,Numbering,norx,Ux,Uy,Px,Py,...
...
for i=1:2
    [X,Y] = CP2Phsical(noe,Numbering,norx,Ux,Uy,Px{i},Py{i},nx,ny);
    ax(i)=subplot(2,2,i);
    hold(ax(i),'on');
...
nx=size(Load{z},1);
ny=size(Load{z},2);
[X,Y] = CP2Phsical(noe,Numbering,norx,Ux,Uy,Px{i-2},Py{i-2},nx-1,ny-1);
hold on;
 ii=1;
